...
class KotlinBLEBridgeTest {
    ...
    private fun addMockPeer(
        ...
        val peerConnectionClass =
            Class.forName(
                "network.columba.app.reticulum.ble.bridge.KotlinBLEBridge\$PeerConnection",
            )
        val deduplicationStateClass =
            Class.forName(
                "network.columba.app.reticulum.ble.bridge.KotlinBLEBridge\$DeduplicationState",
            )
    ...
    fun `pending connection notification uses current peer state after deduplication`() =
        runTest {
            ...
            val pendingConnectionClass =
                Class.forName(
                    "network.columba.app.reticulum.ble.bridge.KotlinBLEBridge\$PendingConnection",
                )
