...
must_haves:
  truths:
    - "Opus packets from Kotlin are decodable by Python pyogg with intelligible audio"
...
<objective>
Implement Opus codec in Kotlin with all 9 quality profiles, producing decode-compatible output with Python LXST.
...
Output: Working Opus.kt with encoder/decoder and unit tests verifying packets are decodable by Python pyogg.
</objective>
...
<task type="auto">
  <name>Verify opus-jni library availability and API</name>
  <files></files>
  <action>
Before implementation, verify the opus-jni library:

1. Search Maven Central for exact artifact coordinates:
```bash
# Search for opus-jni on Maven Central
curl -s "https://search.maven.org/solrsearch/select?q=opus+jni+android&rows=20&wt=json" | jq '.response.docs[] | {id, latestVersion}'
```

2. If not on Maven Central, check JitPack or GitHub:
   - `io.rebble.cobble:opus-jni` - https://github.com/nicholaskell/opus-jni
   - Alternative: `com.github.nicholaskell:opus-jni`

3. Find and document the actual API:
   - Encoder class name (OpusEncoder? Opus? EncoderApi?)
   - Decoder class name
   - Initialization method (constructor params: sampleRate, channels, application)
   - Encode method signature
   - Decode method signature

4. If library unavailable, report blocker immediately with alternatives found.

**DO NOT proceed to implementation with placeholder API names.**
  </action>
  <verify>
1. Library coordinates confirmed (groupId:artifactId:version)
2. API class names documented
3. Encode/decode method signatures documented
  </verify>
  <done>Opus library coordinates and API verified. Ready for implementation.</done>
</task>
...
**Critical:** Opus encoder output MUST be decode-compatible with Python pyogg. This means:
- Python pyogg can decode Kotlin-encoded packets and produce intelligible audio
- Kotlin can decode Python pyogg-encoded packets and produce intelligible audio
- Byte-identical output is NOT required (different encoder implementations produce different bytes)
  </action>
  <verify>
...
<success_criteria>
- Opus encoder produces decode-compatible output (packets decodable by Python pyogg with intelligible audio)
- All 9 profiles (VOICE_LOW through AUDIO_MAX) encode successfully
- Encoded packets respect bitrate ceilings
- Encoder/decoder round-trip preserves audio fidelity (>90% correlation)
- Cross-implementation decode test: Kotlin encode → Python decode → audio sounds correct
