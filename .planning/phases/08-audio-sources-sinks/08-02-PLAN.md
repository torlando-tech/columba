 ...
 *
 * Phase 8 boundary: This source pushes encoded bytes to sink.
 * Phase 9-10 will wire sink -> network -> remote decode.
 *
...
class LineSource(
     ...
     * 3. Apply gain
     * 4. Encode with codec
     * 5. Push encoded bytes to sink (sink will decode when ready)
     *
     * NOTE: For Phase 8 testing, create a loopback test that explicitly
     * wires source→codec.encode→codec.decode→sink. In production (Phase 9),
     * the decode step happens on remote side after network transmission.
     */
    private suspend fun ingestJob() {
        ...
        while (isRunningFlag.get()) {
            ...
            // Push encoded bytes to sink with backpressure check (Python LXST Sources.py:260-263)
            val currentSink = sink
            if (currentSink != null && currentSink.canReceive(this)) {
                // Phase 8: Push encoded bytes. Sink is responsible for decode/playback.
                // For Phase 8 testing, create explicit loopback: encode→decode→sink.handleFrame()
                // For Phase 9 production, decode happens on remote side.
...
1. **Filters in bridge**: 08-RESEARCH.md recommends letting bridge filters run (already <1ms). LineSource doesn't add extra filters. Bridge applies BPF/LPF internally before readAudio() returns.
2. **Push model**: Matches Python - source calls sink.handleFrame(), not sink pulling
3. **Phase 8 boundary**: Source pushes encoded bytes. For testing, create explicit loopback test. For production (Phase 9), decode happens on remote side after network transmission.
