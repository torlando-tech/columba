...
- The 30-second cooldown after selection - still valuable for stability
  ...
  <verify>
1. Run: `grep -n "SharingStarted.WhileSubscribed" app/src/main/java/network.columba.app/service/PropagationNodeManager.kt`
   Expected: 3 matches (lines ~202, ~212, ~236)

2. Run: `grep -n "SharingStarted.Eagerly" app/src/main/java/network.columba.app/service/PropagationNodeManager.kt`
...
2. Run: `cd /home/tyler/repos/public/columba-issue-342-map-location-dialog && JAVA_HOME=/home/tyler/android-studio/jbr ./gradlew :app:testDebugUnitTest --tests "network.columba.app.service.PropagationNodeManagerTest"`
...
DO NOT modify this code - just verify it's present.
  ...
  <verify>
Run: `grep -n "Relay selection loop detected" app/src/main/java/network.columba.app/service/PropagationNodeManager.kt`
Expected: 1 match showing Sentry.captureMessage call (line ~841)

Run: `grep -n "recordSelection" app/src/main/java/network.columba.app/service/PropagationNodeManager.kt`
...
1. **Code verification:**
   - `grep -c "WhileSubscribed(5000L)" app/src/main/java/network.columba.app/service/PropagationNodeManager.kt` returns 3
   - `grep -c "SharingStarted.Eagerly" app/src/main/java/network.columba.app/service/PropagationNodeManager.kt` returns 0
