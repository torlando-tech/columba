...
|----------|----------|--------|---------|
| `data/src/main/java/network.columba.app/data/db/dao/AnnounceDao.kt` | deleteAllAnnouncesExceptContacts(identityHash) SQL query | VERIFIED | Lines 143-153: SQL DELETE with NOT IN subquery against contacts table |
| `data/src/main/java/network.columba.app/data/repository/AnnounceRepository.kt` | deleteAllAnnouncesExceptContacts(identityHash) passthrough | VERIFIED | Lines 389-391: Method exists, calls DAO correctly |
| `app/src/main/java/network.columba.app/viewmodel/AnnounceStreamViewModel.kt` | Updated deleteAllAnnounces() with identity-aware routing | VERIFIED | Lines 472-488: Gets active identity, routes to new method, falls back to old method if no identity |
| `app/src/main/java/network.columba.app/ui/screens/AnnounceStreamScreen.kt` | Updated dialog text mentioning contact preservation | VERIFIED | Line 854: "except those saved in My Contacts" |
| `data/src/test/java/network.columba.app/data/db/dao/AnnounceDaoTest.kt` | DAO-level tests for deleteAllAnnouncesExceptContacts SQL query | VERIFIED | Lines 335-457: 6 tests covering all edge cases, all PASSED |
| `app/src/test/java/network.columba.app/viewmodel/AnnounceStreamViewModelTest.kt` | ViewModel-level tests for identity-aware delete routing | VERIFIED | Lines 869-924: 3 tests covering routing logic, all PASSED |
