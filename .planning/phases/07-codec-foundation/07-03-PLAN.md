...
must_haves:
  truths:
    - "Codec2 packets from Kotlin are decodable by Python pycodec2 with intelligible audio"
...
<objective>
Implement Codec2 codec in Kotlin with all 7 modes, producing decode-compatible output with Python LXST.

Purpose: Enable ultra-low-bitrate voice encoding (700 bps through 3200 bps) for bandwidth-constrained voice calls.

Output: Working Codec2.kt with encoder/decoder and unit tests verifying packets are decodable by Python pycodec2.
</objective>
...
<task type="auto">
  <name>Verify codec2-android library availability and API</name>
  <files></files>
  <action>
Before implementation, verify the codec2-android library:

1. Search Maven Central for exact artifact coordinates:
```bash
# Search for codec2 on Maven Central
curl -s "https://search.maven.org/solrsearch/select?q=codec2+android&rows=20&wt=json" | jq '.response.docs[] | {id, latestVersion}'
```

2. Check UstadMobile's GitHub for actual API:
   - https://github.com/nicholaskell/Android-Codec2
   - https://github.com/nicholaskell/JCodec2

3. Find and document the actual API:
   - Encoder/Codec class name
   - Mode constants (how modes are represented)
   - Initialization method (constructor params)
   - `samplesPerFrame()` method availability
   - `bytesPerFrame()` method availability
   - Encode method signature (short[]? byte[]?)
   - Decode method signature

4. If library unavailable, check for alternatives:
   - Build from source and include as local AAR
   - JitPack alternative
...
4. Mode constants verified
  </verify>
  <done>Codec2 library coordinates and API verified. Ready for implementation.</done>
</task>
...
<success_criteria>
- Codec2 encoder produces decode-compatible output (packets decodable by Python pycodec2 with intelligible audio)
- All 7 modes (700C through 3200) encode successfully
- Encoded packets have correct header byte prepended (0x00-0x06)
